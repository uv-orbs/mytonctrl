...
def Init():
    ...
    console.AddItem(
        "update", Enable, "Update some function: 'JR' - jsonrpc.  Example: 'update JR'")
    ...
    console.AddItem("setwebpass", SetWebPassword,
                    "Set a password for the web admin interface")
...
def Refresh():
    ...
    local.buffer["mconfigPath"] = "/home/{user}/.local/share/mytoncore/mytoncore.db".format(
        user=user)
    if user == 'root':
    ...
    local.buffer["validatorAppPath"] = tonBinDir + \
...
def Enable(args):
    ...
    args = ["python3", local.buffer["myPath"], "-u",
            user, "-e", "enable{name}".format(name=name)]
    RunAsRoot(args)
# end define
...
def CreateLocalConfig(initBlock, localConfigPath=defaultLocalConfigPath):
    ...
    data["validator"]["init_block"]["root_hash"] = hex2base64(
        initBlock["rootHash"])
    data["validator"]["init_block"]["file_hash"] = hex2base64(
        initBlock["fileHash"])
...
def FirstNodeSettings():
    ...
    if os.path.isfile(vconfigPath):
        local.AddLog(
            "Validators config.json already exist. Break FirstNodeSettings fuction", "warning")
        return
    # end if
    ...
    if vuser not in text:
        ...
        args = ["/usr/sbin/useradd", "-d",
                "/dev/null", "-s", "/dev/null", vuser]
        subprocess.run(args)
    # end if
    ...
    args = [validatorAppPath, "--global-config", globalConfigPath,
            "--db", tonDbDir, "--ip", addr, "--logname", tonLogPath]
    subprocess.run(args)
...
def DownloadDump():
    ...
    # download dump
    cmd = "curl -s {url}/dumps/latest.tar.lz | pv | plzip -d -n8 | tar -xC /var/ton-work/db".format(
        url=url)
    os.system(cmd)
# end define
...
def FirstMytoncoreSettings():
    ...
    Add2Systemd(name="mytoncore", user=user,
                start="/usr/bin/python3 /usr/src/mytonctrl/mytoncore.py")
    ...
    if os.path.isfile(path) or os.path.isfile(path2):
        local.AddLog(
            "mytoncore.db already exist. Break FirstMytoncoreSettings fuction", "warning")
        return
    # end if
...
def EnableValidatorConsole():
    ...
    if os.path.isfile(server_key) or os.path.isfile(client_key):
        local.AddLog(
            "Server or client key already exist. Break EnableValidatorConsole fuction", "warning")
        return
    # end if
    ...
    # chown 2
    args = ["chown", user + ':' + user,
            server_pubkey, client_key, client_pubkey]
    subprocess.run(args)
    ...
    validatorConsole["appPath"] = tonBinDir + \
    ...
    cmd = "python3 {srcDir}mytonctrl/mytoncore.py -e \"enableVC\"".format(
        srcDir=srcDir)
...
def EnableLiteServer():
    ...
    if os.path.isfile(liteserver_pubkey):
        local.AddLog(
            "Liteserver key already exist. Break EnableLiteServer fuction", "warning")
        return
    # end if
...
def BackupVconfig():
    local.AddLog(
        "Backup validator config file 'config.json' to 'config.json.backup'", "debug")
    vconfigPath = local.buffer["vconfigPath"]
...
def GetPortsFromVconfig():
    ...
    mconfig["validatorConsole"]["addr"] = "127.0.0.1:{}".format(
        mconfig["control"][0]["port"])
...
def DangerousRecoveryValidatorConfigFile():
    ...
    for item in keyring:
        ...
        pubkey = pubkeyObject.public_bytes(
...
def CreateSymlinks():
    ...
    file.write(
        "/usr/bin/ton/lite-client/lite-client -C /usr/bin/ton/global.config.json $@")
    file.close()
    if cport:
...
def EnableDhtServer():
    ...
    if os.path.isfile("/var/ton-dht-server/config.json"):
        local.AddLog(
            "DHT-Server config.json already exist. Break EnableDhtServer fuction", "warning")
        return
    # end if
    ...
    cmd = cmd.format(dht_server=dht_server,
                     globalConfigPath=globalConfigPath, tonDhtServerDir=tonDhtServerDir)
    ...
    # Первый запуск
    args = [dht_server, "-C", globalConfigPath,
            "-D", tonDhtServerDir, "-I", addr]
    ...
    args = [generate_random_id, "-m", "dht",
            "-k", tonDhtKeyringDir + key, "-a", text]
...
def EnableJsonRpc():
    ...
    user = local.buffer["user"]
    exitCode = RunAsRoot(
        ["bash", "/usr/src/mytonctrl/scripts/jsonrpcinstaller.sh", "-u", user])
    if exitCode == 0:
...
def EnablePytonv3():
    ...
    user = local.buffer["user"]
    exitCode = RunAsRoot(
        ["bash", "/usr/src/mytonctrl/scripts/pytonv3installer.sh", "-u", user])
    if exitCode == 0:
